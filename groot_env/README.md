...
4. **No steering support.** The `--steer` path in `src/openpi/serving/steering.py` targets the pi0.5 PyTorch action expert. TODO: port to GR00T — the DiT backbone has different hook attach points and activation dimensionality, so `ConceptorSteeringHook` wiring, NPZ key schema, and per_step handling all need to be revisited. Separate effort from the current pi0.5 steering work.
