#!/usr/bin/env python3
"""
Transfer Experiment (Design B) — pi0.5 LIBERO
==============================================

For a fixed TARGET task j, sweeps all SOURCE tasks i and applies each source's
best conceptor (global and per-step) to j. Layer is held at j's best L per
strategy; aperture alpha and steering beta come from source i's best recipe.

For each (i, j):
  * Global:      C = {i}__L{L_j^global}__{alpha_i^global}__C_contrastive,
                 beta = beta_i^global
  * Per-step:    for step in [0, 9] (full time-indexed stack):
                 C = {i}__L{L_j^per_step}__per_step_{step}__C_contrastive,
                 beta = beta_i^per_step

Reuses infra from conceptor_steering.py (hook, server, eval subprocess, npz).
"""
...
def extract_best_configs(summary_path: pathlib.Path):
    """From a task's summary.json, return best global and best per-step recipe."""
...
class Args:
    """Transfer: apply each source task's best conceptor onto a fixed target."""

    target_task: str = LIBERO_TASKS[0]
    config: str = "pi05_libero"
    checkpoint_dir: str = (
        "/vast/projects/ungar/stellar/miaom/openpi-metaworld/checkpoints/pi05_libero/libero_b200_bs512/2000"
    )
    task_suite_name: str = "libero_10"
    num_episodes: int = 15
    port: int = 8100

    results_root: str = "experiments/pi05_libero/steering_results"
    output_dir: str = "experiments/pi05_libero/transfer_results"

    # Per-step time-bucket stack (npz has these steps precomputed).
    per_step_values: List[int] = dataclasses.field(default_factory=lambda: [0, 9])


def main(args: Args):
    target = args.target_task
    target_short = target[:60]
    target_out = pathlib.Path(args.output_dir) / f"target_{target_short}"
    ...
    best_configs = {}
    for t in LIBERO_TASKS:
        summary_p = results_root / t[:60] / "summary.json"
    ...
    for source in sources:
        ...
        for step in args.per_step_values:
            idx += 1
            cond_name = f"source_{source_short}__per_step_{step}"
    ...
    save_progress()
    logger.info(f"\n{'='*70}\nDone target={target_short}: {len(all_cells)} cells\n{'='*70}")
