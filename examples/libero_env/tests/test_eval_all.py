...
Like ``test_libero_env.py`` they run inside ``examples/libero_env/.venv``.
...
class TestSuccessRateRegex:
    ...
    def test_parses_full_success(self) -> None:
        matches = eval_all.SUCCESS_RATE_RE.findall(
            "[libero_spatial/pick_up_the_black_bowl/task_00] success_rate=1.00 (1/1)"
        )
        assert matches == ["1.00"]
...
class TestBuildCommand:
    ...
    def test_collect_flag_forwarded_when_true(self) -> None:
        cmd = eval_all._build_command(
            _default_args(collect=True), 0, self._TEST_OUTPUT_DIR
        )
        assert "--collect" in cmd

    def test_collect_flag_absent_when_false(self) -> None:
        cmd = eval_all._build_command(
            _default_args(collect=False), 0, self._TEST_OUTPUT_DIR
        )
    ...
    def test_output_dir_always_forwarded(self) -> None:
        ...
        prevents main.py from falling back to its own ``output/single-{suite}/``
...
class TestArgsDefaults:
    ...
    def test_default_values(self) -> None:
        ...
        assert args.task_suite_name == "libero_spatial"
