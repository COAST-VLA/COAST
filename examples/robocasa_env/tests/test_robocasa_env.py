...
class TestArgs:
    def test_defaults(self) -> None:
        ...
        assert args.fps == 24
        assert args.seed == 7
        # Activation collection is opt-in.
...
class TestEvalTaskSignature:
    ...
    def test_signature(self) -> None:
        """eval_task takes (env_name, policy, args, output_dir, [collect_session]).

        collect_session is keyword-only with a default of None — eval_all.py
        and main.py both rely on this so they don't have to pass it when the
        --collect flag is off.
        """
