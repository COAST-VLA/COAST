"""Tests for eval_all.py's parallel subprocess orchestration.

Covers the pure-Python units of eval_all.py without spinning up a policy
server or a real RoboCasa env:
...
None of these tests need a running policy server, MuJoCo, or the robocasa
task registry. They can run on a CPU-only machine in well under 10 seconds.
Like ``test_robocasa_env.py`` they run inside ``examples/robocasa_env/.venv``.
...
class TestSuccessRateRegex:
    ...
    ``eval_task``: ``[<env_name>/<split>] success_rate=X.XX (k/N)``.
    ...
    def test_parses_partial_success(self) -> None:
        matches = eval_all.SUCCESS_RATE_RE.findall(
            "[OpenCabinet/target] success_rate=0.67 (2/3)"
        )
        assert matches == ["0.67"]
    ...
    def test_last_match_wins(self) -> None:
        ...
        log = textwrap.dedent(
            """\
            INFO:__main__:[CloseBlenderLid/pretrain] success_rate=0.00 (0/1)
            INFO:__main__:some intermediate noise
            INFO:__main__:[CloseBlenderLid/pretrain] success_rate=1.00 (1/1)
            """
        )
...
class TestBuildCommand:
    ...
    def test_includes_main_py_and_python_executable(self) -> None:
        ...
        cmd = eval_all._build_command(
            args, env_name="CloseBlenderLid", output_dir=self._TEST_OUTPUT_DIR
        )
    ...
    def test_required_main_flags_are_forwarded(self) -> None:
        ...
        cmd = eval_all._build_command(
            args, env_name="OpenCabinet", output_dir=self._TEST_OUTPUT_DIR
        )
        # Every flag-value pair should appear somewhere in the argv list.
        # NOTE: robocasa main.py does NOT have --num_steps_wait (unlike libero),
        # and does not take --task_set (eval_all dispatches per-env_name).
        expected_pairs = {
            "--host": "1.2.3.4",
            "--port": "9999",
    ...
    def test_num_steps_wait_not_forwarded(self) -> None:
        """robocasa main.py has no --num_steps_wait arg — forwarding it would
        crash tyro with 'unrecognized argument'. The flag must never appear
        regardless of Args configuration."""
        cmd = eval_all._build_command(
            _default_args(),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        )
        assert "--num_steps_wait" not in cmd

    def test_env_name_is_forwarded(self) -> None:
        """The env_name argument must land in the argv as --env_name <name>."""
        cmd = eval_all._build_command(
            _default_args(),
        ...
        assert cmd[cmd.index("--env_name") + 1] == "TurnOnMicrowave"
        # The flag must appear exactly once.
        assert cmd.count("--env_name") == 1

    def test_split_is_forwarded(self) -> None:
        cmd = eval_all._build_command(
            _default_args(split="target"),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        )
        assert "--split" in cmd
        assert cmd[cmd.index("--split") + 1] == "target"

    def test_env_name_is_the_only_thing_that_changes_across_calls(self) -> None:
        args = _default_args()
        ...
        # Both should differ only at the --env_name value position.
        ...
        diff_idx = diffs[0]
        assert cmd_a[diff_idx - 1] == "--env_name"
        assert cmd_a[diff_idx] == "CloseBlenderLid"
        assert cmd_b[diff_idx] == "OpenCabinet"

    def test_collect_flag_forwarded_when_true(self) -> None:
        cmd = eval_all._build_command(
            _default_args(collect=True),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        )
        assert "--collect" in cmd

    def test_collect_flag_absent_when_false(self) -> None:
        cmd = eval_all._build_command(
            _default_args(collect=False),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        )
    ...
    def test_output_dir_forwarded_verbatim_without_task_set_nesting(self) -> None:
        ...
        cmd = eval_all._build_command(
            _default_args(task_set="composite_seen", split="target"),
            env_name="CloseBlenderLid",
            output_dir="/foo/bar",
        )
    ...
    def test_render_cameras_default_uses_space_separated_form(self) -> None:
        """Default is ``['agentview_left', 'agentview_right', 'eye_in_hand']``
        and tyro's ``List[str]`` form is ``--render_cameras cam1 cam2 cam3``
        (single flag, positional values). The repeated-flag form silently
        drops all but the last value."""
        cmd = eval_all._build_command(
            _default_args(),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        ...
        assert cmd[flag_idx + 1] == "agentview_left"
        assert cmd[flag_idx + 2] == "agentview_right"
        assert cmd[flag_idx + 3] == "eye_in_hand"
    ...
    def test_render_cameras_custom_single_value(self) -> None:
        cmd = eval_all._build_command(
            _default_args(render_cameras=["agentview_left"]),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        )
        flag_idx = cmd.index("--render_cameras")
    ...
    def test_render_cameras_empty_list_omits_flag(self) -> None:
        ...
        cmd = eval_all._build_command(
            _default_args(render_cameras=[]),
            env_name="CloseBlenderLid",
            output_dir=self._TEST_OUTPUT_DIR,
        )
        assert "--render_cameras" not in cmd

    def test_collect_output_dir_render_cameras_compose_without_interleaving(
        self,
    ) -> None:
        """All three optional flags together must not interleave each other's
        values. This is the scenario that would break if render_cameras used
        ``cmd.extend([...])`` and a later optional flag landed mid-list."""
        args = _default_args(
            collect=True,
            render_cameras=["agentview_left", "agentview_right", "eye_in_hand"],
            max_steps=200,
        )
        cmd = eval_all._build_command(
            args, env_name="CloseBlenderLid", output_dir="/tmp/x"
        )
        # All four must be present (render_cameras, collect, max_steps, output_dir)
        assert "--collect" in cmd
        assert "--render_cameras" in cmd
        assert "--max_steps" in cmd
        assert "--output_dir" in cmd
        # --render_cameras values must be immediately followed by its camera
        # names, not a stray flag like --collect.
        rc = cmd.index("--render_cameras")
        assert cmd[rc + 1] == "agentview_left"
        assert cmd[rc + 2] == "agentview_right"
        assert cmd[rc + 3] == "eye_in_hand"
...
# configured code. No robocasa imports, no MuJoCo — just an argv-parsing shim
# so main.py's flags (which eval_all forwards) don't crash argv handling.

_FAKE_MAIN_SUCCESS = """
    import sys, logging
    # Swallow all CLI args; we don't need to validate them here.
    logging.basicConfig(level=logging.INFO)
    logging.info('[fake_env/pretrain] success_rate=1.00 (1/1)')
    sys.exit(0)
"""
...
_FAKE_MAIN_FAILURE = """
    import sys, logging
    logging.basicConfig(level=logging.INFO)
    logging.info('[fake_env/pretrain] success_rate=0.00 (0/1)')
    sys.exit(0)
"""
...
class TestRunOneTask:
    ...
    The fake main.py is Python-only: no robocasa, no MuJoCo, no policy server.
    """
    ...
    def test_parses_success_rate_from_subprocess_stdout(
        ...
        result = eval_all._run_one_task(
            _default_args(task_set="atomic_seen"),
            env_name="fake_env",
            task_idx=0,
            log_dir=str(log_dir),
            cwd=str(fake_dir),
        ...
        assert result["env_name"] == "fake_env"
    ...
    def test_per_task_log_filename_uses_task_idx_and_env_name(
        self, tmp_path: pathlib.Path
    ) -> None:
        """Two tasks in the same log_dir must produce distinct log files.
        The zero-padded task_idx prefix keeps files sorted in submission order
        even when env_names are alphabetical."""
        fake_dir, log_dir, output_dir = _make_fake_env(tmp_path, _FAKE_MAIN_SUCCESS)
        ...
        assert "CloseBlenderLid" in r0["log_path"]
        assert "task_05" in r5["log_path"]
        assert "OpenCabinet" in r5["log_path"]
