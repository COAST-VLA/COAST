...
1. On a machine with a powerful GPU (~NVIDIA 4090), clone and install the COAST repository following the instructions in the root README.
2. Start the COAST policy server via the following command:
...
3. Clone the COAST repo and install the openpi client, which we will use to connect to the policy server (this has very few dependencies and should be very fast to install): with the DROID conda environment activated, run `cd $COAST_ROOT/packages/openpi-client && pip install -e .`.
